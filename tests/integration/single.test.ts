...
describe("Single Render Endpoint", () => {
  it("should render model with default settings and print metadata", async () => {
    ...
    saveTestImage("single_default.webp", result.data.image);
  }, 100000);
  ...
  it("should have a light blue background (css hex #ADD8E6)", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_light_blue_hex.webp", result.data.image);
  }, 60000);
  ...
  it("should render with a background color", async () => {
    ...
    saveTestImage("single_fox_orange_bg.webp", result.data.image);
  }, 60000);
  ...
  it("should have a light blue gradient background (linear-gradient)", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_light_blue_linear-gradient.webp", result.data.image);
  }, 60000);
  ...
  it("should render with an environment image", async () => {
    ...
    saveTestImage("single_sofa_hdri_env.webp", result.data.image);
  }, 120000);
  ...
  it("should have a light blue gradient background (radial-gradient)", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_light_blue_radial-gradient.webp", result.data.image);
  }, 60000);

  it("should render with a skybox image and variant", async () => {
    const result = await makeRequest("/single", {
      model: MATERIALS_VARIANTS_SHOE,
      skybox: HDRI,
      skyboxHeight: "1.5m",
      variantName: "Beach",
    });
    ...
    saveTestImage("single_shoe_skybox_variant.webp", result.data.image);
  }, 120000);
  ...
  it("should have a light blue background (hsl)", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_light_blue_hsl.webp", result.data.image);
  }, 60000);
  ...
  it("should render with custom scale and avif output", async () => {
    ...
    saveTestImage("single_shoe_scaled.avif", result.data.image);
  }, 60000);
  ...
  it("should render with PNG output format", async () => {
    ...
    saveTestImage("single_png.png", result.data.image);
  }, 60000);
  ...
  it("should render with custom orientation and background gradient", async () => {
    ...
    saveTestImage(
      "single_rotated_45deg_radial-gradient.webp",
      result.data.image
    );
  }, 60000);

  it("should render fox with HDRI skybox passed as base64", async () => {
    const hdriBase64 = await urlToBase64(HDRI);
    const result = await makeRequest("/single", {
      model: FOX,
      skybox: hdriBase64,
    });
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_fox_hdri_base64.webp", result.data.image);
  }, 120000);

  it("should handle base64 model input", async () => {
    const base64Model = await urlToBase64(ANIMATED_MORPH_CUBE);
    ...
    saveTestImage("single_base64_input.webp", result.data.image);
  }, 90000);
  ...
  it("should render with custom orientation, tone mapping, and dimensions", async () => {
    ...
    saveTestImage("single_shoe_custom_settings.png", result.data.image);
  }, 120000);
  ...
  it("should render with custom dimensions and background image", async () => {
    ...
    saveTestImage("single_512x512_background_image.webp", result.data.image);
  }, 60000);
  ...
  it("should have a light blue background (css lightblue)", async () => {
    ...
    expect(result.status).toBe(200);
    expect(result.data.success).toBe(true);
    saveTestImage("single_light_blue.webp", result.data.image);
  }, 60000);
});
